...
const getAnimals = (formData = {}) => {
  ...
  // standard fetch request usage for making a GET request
  fetch(queryUrl)
  .then (response => {
    if(!response.ok) {
      // since this portion will be used by zookeepers, we can keep it as an alert
      // then therefore can convert it into a modal as well, aka "toast" dialog box
